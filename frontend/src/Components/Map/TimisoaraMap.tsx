...
// Funcție helper pentru verificarea "point in polygon" folosind ray casting
...
function isPointInPolygon(lat: number, lng: number, feature: GeoJSON.Feature<GeoJSON.Polygon | GeoJSON.MultiPolygon>): boolean {
  ...
  const polygons: number[][][] = feature.geometry.type === "Polygon"
    ? [feature.geometry.coordinates[0]] // doar outer ring (găurile le vom trata separat)
    : feature.geometry.coordinates.map((poly) => poly[0]);

  const holes: number[][][] = feature.geometry.type === "Polygon"
    ? feature.geometry.coordinates.slice(1) // găuri din poligon
    : feature.geometry.coordinates.flatMap((poly) => poly.slice(1));

  // Ray casting pe un ring (array de coordonate [lng, lat])
  ...
  // Dacă este în interiorul oricărui outer ring și NU este în nicio gaură, e valid
